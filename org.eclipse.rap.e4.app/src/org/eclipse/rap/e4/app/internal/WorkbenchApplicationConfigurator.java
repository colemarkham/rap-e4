...
import org.eclipse.rap.e4.app.internal.branding.AbstractBranding;
import org.eclipse.rap.e4.app.internal.branding.BrandingExtension;
import org.eclipse.rap.e4.app.internal.branding.BrandingManager;
import org.eclipse.rap.e4.app.internal.branding.BrandingUtil;
...
//      WorkbenchPlugin.log( cex.getStatus() );
    	System.err.println(cex.getStatus());
    }
    return result;
  }
  ...
  private void registerEntryPoints( Application application ) {
    for( IConfigurationElement element : getProductExtensions() ) {
      String id = element.getAttribute( "id" );
//      String path = element.getAttribute( "path" );
//      String className = element.getAttribute( "class" );
      String applicationId = element.getAttribute( "application" );
      String appName = id;
      IConfigurationElement[] productPropElements = element.getChildren("property");
      Map<String, String> productProperties = new HashMap<String, String>(productPropElements.length);
      for (IConfigurationElement prop: productPropElements) {
...
//        } else if( applicationId != null ) {
        Bundle brandingBundle = Platform.getBundle(element.getContributor().getName());
          entryPointFactory = createEntryPointFactoryForApplication( applicationId, brandingBundle, productProperties );
//        } else {
//          throw new IllegalArgumentException( "Neither class nor applicationId specified" );
//        }
        Map<String, String> brandingProperties = getBrandingProperties( brandingId );
        application.addEntryPoint( path, entryPointFactory, brandingProperties );
      } catch( final Throwable thr ) {
        String text = "Could not register entry point ''{0}'' with id ''{1}''.";
        Object[] param = new Object[] { path, id };
  ...
  @SuppressWarnings( "unchecked" )
  private EntryPointFactory createEntryPointFactoryForApplication( String applicationId, Bundle brandingBundle, Map<String, String> props )
    throws ClassNotFoundException
  {
    ...
    return createApplicationEntryPointFactory( applicationClass, brandingBundle, props );
  }

  private static EntryPointFactory
    createApplicationEntryPointFactory( final Class<? extends IApplication> applicationClass, final Bundle brandingBundle, final Map<String, String> props )
  {
    return new EntryPointFactory() {
      public EntryPoint create() {
        return new EntryPointApplicationWrapper( applicationClass, brandingBundle, props );
      }
    };
  }
...
//    	  System.err.println(ce.getStatus());
//      }
//    }
  }
  ...
  private static IConfigurationElement[] getProductExtensions() {
    IExtensionRegistry registry = Platform.getExtensionRegistry();
    IExtensionPoint extensionPoint = registry.getExtensionPoint( PI_RUNTIME + "." + PT_PRODUCTS );
